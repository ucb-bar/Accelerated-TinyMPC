...
#if defined(USE_RVV) || defined(USE_RVVU) || defined(USE_CPU)  || defined(USE_GEMMINI)
    // Column if ColMajor
    Scalar_ *col(int col) {
...
const static spad_ptr_t zero_spad = nrI_spad + DIM;
const static spad_ptr_t Kinf_spad = zero_spad + DIM;
...
const static spad_ptr_t temp_spad = BANK_ROWS*3/4;
#endif
typedef struct
{
    tinytype rho;
