...
#include "trajectory_data/quadrotor_50hz_y_axis_line.hpp"
...
extern "C"
{
    ...
    {
        if (argc != 14) {
            std::cerr << "Usage: " << argv[0] << " [13 floating point values]" << std::endl;
            return 1;
        }
        ...
        // 2. Update reference 
        work.Xref = Xref_total.block<NSTATES, NHORIZON>(0, time);
        ...
        // std::cout << "Time for iter " << time << ": " << time1 << std::endl;
...
//         // 2. Update reference 
//         work.Xref = Xref_total.block<NSTATES, NHORIZON>(0, i);
